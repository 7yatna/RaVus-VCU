	...
	if(opmode==MOD_OFF)
		{
			...
			{
				DigIo::NEG_CON.Toggle();
				DigIo::PRE_CON.Toggle();
				DigIo::POS_CON.Toggle();
			}
		}
...
/*
	...
	*/
    ...
    case MOD_OFF:
        initbyStart=false;
        initbyCharge=false;
        DigIo::INV_PWR.Clear();//inverter power off
		DigIo::POS_CON.Clear();
		DigIo::NEG_CON.Clear();
		DigIo::CPMP_PWR.Clear();
		DigIo::PRE_CON.Clear();							
...
*/
    ...
    case MOD_PRECHARGE:
        if (!chargeMode)
        {
            if(selectedInverter != &openInv)DigIo::INV_PWR.Set();//inverter power on but not if we are in charge mode and not if OI
        }
        else if((Param::GetInt(Param::ShuntType) == 0) && selectedInverter == &leafInv)//Shunt 0 + Leaf is precharge using leaf inverter voltage
        {
            DigIo::INV_PWR.Set(); //inverter power on
        }
        DigIo::NEG_CON.Set();
		DigIo::CPMP_PWR.Set();
        ...
        //if(rlyDly==0) 
		  DigIo::PRE_CON.Set();//commence precharge
    ...
    case MOD_PCHFAIL:
        StartSig=false;
        DigIo::PRE_CON.Clear();//explicitly turn off precharge relay in a fail condition
