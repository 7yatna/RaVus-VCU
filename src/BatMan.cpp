...
void BATMan::StateMachine()
{
    ...
    case 8: //Waiting State
    {
        ...
        if(IdleCnt > cycletime)
        {
            ...
            IdleCnt = 0;
            LoopRanCnt++;
            Param::SetInt(Param::BMS_LoopCnt, LoopRanCnt);
        }
        break;
    }
    ...
    Param::SetInt(Param::BMS_LoopState, LoopState);
}
...
void BATMan::GetData(uint8_t ReqID)
{
    ...
    receive1 = spi_xfer(SPI2, ReqData[0]);  // do a transfer
    receive2 = spi_xfer(SPI2, ReqData[1]);  // do a transfer

    //receive1 = spi_xfer(SPI2, Request[0]);  // do a transfer
    // receive2 = spi_xfer(SPI2, Request[1]);  // do a transfer

    for (count2 = 0; count2 <= 72; count2 = count2 + 2)
    {
        receive1 = spi_xfer(SPI2, padding);  // do a transfer
...
void BATMan::WriteCfg()
{
    ...
    {
        receive1 = spi_xfer(SPI2, cfgwrt[cnt]);  // do a transfer
    }
    DigIo::BAT_EN.Set();
}
...
void BATMan::GetTempData ()  //request
{
    padding=0x0000;
    DigIo::BAT_EN.Clear();
    receive1 = spi_xfer(SPI2, reqTemp);  // do a transfer
...
void BATMan::WakeUP()
{
    ...
    {
        DigIo::BAT_EN.Clear();
        receive1 = spi_xfer(SPI2, WakeUp[0]);  // do a transfer
        DigIo::BAT_EN.Set();
    }
}
...
void BATMan::Generic_Send_Once(uint16_t Command[], uint8_t len)
{
    ...
    for (int h = 0; h < len; h++)
    {
        receive1 = spi_xfer(SPI2, Command[h]);  // do a transfer
    }
    DigIo::BAT_EN.Set();
...
void BATMan::upDateCellVolts(void)
{
    ...
    while (h <= 100)
    {
        if(Yc < 14) //Check actual measurement present
        {
            if(Voltage[Xr][Yc] > 10) //Check actual measurement present
            {
                if (CellVMax< Voltage[Xr][Yc])
                {
                    ...
                    Param::SetInt(Param::BMS_CellMax, h+1);
                }
                if (CellVMin > Voltage[Xr][Yc])
                {
                    CellVMin =  Voltage[Xr][Yc];
                    Param::SetInt(Param::BMS_CellMin, h+1);
                }
                ...
                {
                    if((Param::GetFloat(Param::BMS_umin) + BalHys) < Voltage[Xr][Yc])
                    {
        ...
        if(Xr == ChipNum)
        {
            Param::SetFloat(Param::BMS_umax,CellVMax);
            Param::SetFloat(Param::BMS_umin,CellVMin);
            Param::SetFloat(Param::BMS_Delta,CellVMax-CellVMin);
            Param::SetInt(Param::BMS_Present,h);
            h = 100;
            break;
        }
    }
...
//debugging balancing//
    ...
    Param::SetInt(Param::BMS_Balancing, CellBalancing);
//
}
/*
...
void BATMan::upDateAuxVolts(void)
{
    ...
    Param::SetFloat(Param::BMS_udc,0);
	Param::SetFloat(Param::BMS_ChipV1,ChipV[0]*0.001280);
	Param::SetFloat(Param::BMS_ChipV2,ChipV[1]*0.001280);
	Param::SetFloat(Param::BMS_udc,(Param::GetFloat(Param::BMS_ChipV1)+Param::GetFloat(Param::BMS_ChipV2)));
	Param::SetFloat(Param::BMS_ChipV3,ChipV[2]*0.001280);
	Param::SetFloat(Param::BMS_ChipV4,ChipV[3]*0.001280);
	Param::SetFloat(Param::BMS_udc,(Param::GetFloat(Param::BMS_udc)+Param::GetFloat(Param::BMS_ChipV3)+Param::GetFloat(Param::BMS_ChipV4)));
	Param::SetFloat(Param::BMS_ChipV5,ChipV[4]*0.001280);
	Param::SetFloat(Param::BMS_ChipV6,ChipV[5]*0.001280);
	Param::SetFloat(Param::BMS_udc,(Param::GetFloat(Param::BMS_udc)+Param::GetFloat(Param::BMS_ChipV5)+Param::GetFloat(Param::BMS_ChipV6)));
	Param::SetFloat(Param::BMS_ChipV7,ChipV[6]*0.001280);
	Param::SetFloat(Param::BMS_ChipV8,ChipV[7]*0.001280);
	Param::SetFloat(Param::BMS_udc,(Param::GetFloat(Param::BMS_udc)+Param::GetFloat(Param::BMS_ChipV7)+Param::GetFloat(Param::BMS_ChipV8)));
    Param::SetInt(Param::BMS_uavg,(Param::GetFloat(Param::BMS_udc)/Param::GetInt(Param::BMS_Present)*1000));
...
void BATMan::upDateTemps(void)
{
    ...
    for (int g = 0; g < ChipNum; g++)
    {
       ...
       // Param::SetFloat((Param::PARAM_NUM)(Param::Chipt0 + g), tempval2);
        ...
        Param::SetFloat((Param::PARAM_NUM)(Param::BMS_Cellt0_0 + g*2),Temp1[g]);
        Param::SetFloat((Param::PARAM_NUM)(Param::BMS_Cellt0_1 + g*2),Temp2[g]);
    }
    Param::SetFloat(Param::BMS_TempMax,TempMax);
    Param::SetFloat(Param::BMS_TempMin,TempMin);
}
