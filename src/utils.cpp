...
void GetDigInputs(CanHardware* can)
{
    ...
    Param::SetInt(Param::din_forward, DigIo::FOR_in.Get() | ((canio & CAN_IO_FWD) != 0));
    Param::SetInt(Param::din_reverse, DigIo::REV_in.Get() | ((canio & CAN_IO_REV) != 0));
    Param::SetInt(Param::din_bms, (canio & CAN_IO_BMS) != 0);
    Param::SetInt(Param::din_12Vgp, DigIo::GP4_in.Get());
	Param::SetInt(Param::din_WAKE_UP, DigIo::GP5_in.Get());													
}
...
float GetUserThrottleCommand()
{
    ...
    int pot1val = 0; //AnaIn::throttle1.Get();
    int pot2val = 0; //AnaIn::throttle2.Get();
...
float ProcessUdc(int motorSpeed)
{
    ...
    if (udc > udclim)
    {
        ...
        {
            DigIo::POS_CON.Clear();  //In this case, open DC switch
            DigIo::PRE_CON.Clear();  //and
...
{
    uint16_t potdisp = 0; //AnaIn::throttle1.Get();
    uint16_t pot2disp = 0; //AnaIn::throttle2.Get();
